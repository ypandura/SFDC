...
// Accessing all shopping invoices associated with the items in the trigger
List<Shipping_Invoice__C> AllShoppingInvoices = [SELECT id, ShippingDiscount__c, 
...
// This will make it easier to look up a shopping invoice
...
for(Shipping_Invoice__C sc : AllShoppingInvoices)
{
    SIMap.put(sc.id, sc);
}
...
// Process the list of items
    if(trigger.isUpdate)
    {
        ...
        for(Shipping_Invoice__C myOrder : AllShoppingInvoices)
        {
    ...
    { 
        for(Item__c itemToProcess : itemList)
        {
            ...
            // Look up the correct shopping invoice from the ones we got earlier
